...
import { createGeometry, regularVertices_mm } from "./geometry.js";
...
const geometry = createGeometry(state, cv);
const exporters = createExporters({
  state,
  geometry,
  getEntitiesForFeature,
});
const {
  uToMM,
  mmToU,
...
function sampleStepValue(){
  const el = document.getElementById("sampleStep");
  const parsed = Number(el ? el.value : state.gridStep_u);
  return Number.isFinite(parsed) ? parsed : state.gridStep_u;
}
...
function updateLiveCSV(){
  ...
  const step = sampleStepValue();

  if (mode === "segments"){
    el.value = exportSegmentsCSV(activeFeature);
    if (status) status.textContent = `— Segments (CNC)`;
  } else {
    const rows = flattenFeaturePoints(activeFeature, step);
    el.value = exportWaypointsCSV(activeFeature, step);
    if (status) status.textContent = `— Waypoints (${rows.length} rows)`;
  }
}
...
// Points table (flattened)
  const pts = flattenFeaturePoints(activeFeature, sampleStepValue());
...
document.getElementById("export").addEventListener("click", ()=>{
  ...
  try {
    let text = "";
    const sampleStep = sampleStepValue();
    if (mode==="csv") {
      text = exportPointsForFeatures(featureList, sampleStep);
    } else if (mode==="gcode") {
      text = exportGCode(featureList);
    } else if (mode==="pmir") {
      text = exportPseudoMaticIR(featureList, IR_PURPOSE, IR_Z_STRATEGY);
    } else {
      text = exportPseudo(featureList);
    }
  ...
  const snapshot = {
    ...
    exports: {
      active: {
        ...
        waypointsCSV: exportWaypointsCSV(activeFeature, sampleStepValue())
      }
    },
